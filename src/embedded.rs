//! The embedded sources: everything the binary serves or lands, compiled
//! in.
//!
...
//! once, in [`crate::distribution_roots`], read here, by `build.rs` for
//! change tracking, and by the packaging test; a test below holds this
//! module to that inventory.

use include_dir::{Dir, include_dir};

pub use crate::distribution_roots::DISTRIBUTION_ROOTS;
...
/// One file per release that needs an operator step, copied whole into a
/// stage's reference tree for the agent to select from.
pub static GUIDANCE: Dir<'static> = include_dir!("$CARGO_MANIFEST_DIR/guidance");

/// The release history, carried whole for the stage's reference tree.
///
/// Generated by release-plz, so a stage can hand an agent the changelog of
/// the exact binary that staged it. Like the licenses it sits beside the
/// root inventory rather than in it: history is crate metadata, not an
/// authored source, and it names the repository that publishes it.
...
/// The files one distribution root carries, as `(path, bytes)` with the path
...
/// Every embedded file, root by root in inventory order, sorted by path
/// within each root.
///
/// The license files are deliberately absent: they are crate metadata the
/// registry requires, not authored sources, and `rk license` serves them.
#[must_use]
...
#[cfg(test)]
mod tests {
    use super::{DISTRIBUTION_ROOTS, artifacts, root_files};
    ...
    /// bytes; a root declared but not embedded is a name the inventory
    /// declares and nothing serves.
    #[test]
    fn the_inventory_and_the_embed_declare_the_same_roots() {
        ...
        embedded.extend(source.lines().filter_map(|line| {
            ...
            (!name.starts_with("LICENSE") && name != "CHANGELOG.md").then(|| name.to_owned())
        }));
        embedded.sort();
        let mut declared: Vec<String> =
            DISTRIBUTION_ROOTS.iter().map(ToString::to_string).collect();
        declared.sort();
        assert_eq!(
            embedded, declared,
            "src/embedded.rs and src/distribution_roots.rs disagree on the distribution roots"
        );
    }
    ...
    fn the_artifact_list_is_stable_and_complete() {
        ...
        let total: usize = DISTRIBUTION_ROOTS
            .iter()
