...
/// `unsupported-schema` refusal for a receipt outside `rk.stage/2`, and a
...
mod tests {
    ...
    fn a_directory_that_does_not_name_itself_refuses() {
        ...
        std::fs::write(
            ...
            format!(
                r#"{{"schema":"rk.stage/2","stage_root":"{}","target":"/nowhere"}}"#,
                other.display()
            ),
        )
        .expect("writes");
    ...
    fn a_malformed_receipt_refuses_before_anything_is_removed() {
        ...
        for body in [
            format!(
                r#"{{"schema":"rk.stage/2","stage_root":"{}"}}"#,
                candidate.display()
            ),
            format!(
                r#"{{"schema":"rk.stage/2","stage_root":"{}","target":null}}"#,
                candidate.display()
            ),
            format!(
            ...
            format!(
                r#"{{"schema":"rk.stage/2","stage_root":"{}","target":"repo"}}"#,
                candidate.display()
            ),
            format!(
                r#"{{"schema":"rk.stage/2","stage_root":"{}","target":"{}/../candidate/repo"}}"#,
                candidate.display(),
                candidate.display()
            ),
            format!(
