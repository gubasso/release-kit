...
/// The whole hook, byte for byte. No `set -eu` on purpose: the contract
/// is exit 0 always, and the `|| :` plus the final line hold it.
...
# After a merge arrives, report local branches the forge already merged and
# deleted. Prints nothing when there are none, and never blocks a pull.
...
#[cfg(test)]
mod tests {
    /// The body opens with a shebang, carries the marker, names the quiet
    /// prune, and ends by succeeding whatever happened above.
    #[test]
    fn the_hook_body_carries_the_marker_and_never_fails() {
        ...
        assert!(super::HOOK_BODY.contains("rk branches prune --quiet"));
