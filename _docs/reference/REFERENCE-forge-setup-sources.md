...
Re-checked 2026-09-17 in a disposable public repository, because a private repository on a free plan answers `403 Upgrade to GitHub Pro or make this repository public` for the rulesets endpoint, so a private probe cannot be read back at all. That repository has since been deleted, so every outcome it established is stated here rather than linked, and a reader who needs it again installs the two-ruleset shape below in a fresh repository. The probe settles four outcomes and one shape. `bypass_actors` is a ruleset field, beside `rules` and never inside one, so an actor named there is excused from every rule the ruleset carries. Over a trunk holding `deletion` and `non_fast_forward` in a ruleset naming nobody, and `pull_request` and `required_status_checks` with the strict policy in a ruleset naming repository-role actor `5`: the administrator's fast-forward push was accepted while the forge printed the unmet check as a note; the administrator's force-push was refused with `Cannot force-push to this branch`; the administrator's deletion of a covered branch was refused with `Repository rule violations found / Cannot delete this branch`; and the Actions installation token, which is an App installation token like any release App's, was refused at the merge endpoint with `405 Repository rule violations found / Required status check "gate" is expected.` while the request stood at `OPEN BEHIND`. The administrator merged that same stale request through the same endpoint, which is the cost this separation carries. The probe also shows that a merge attempt through the forge CLI proves nothing about a ruleset: the CLI refuses a stale request from its own mergeability read, before any request reaches the forge.
...
Bearing: `forge-setup:a-step-is-idempotent` and `forge-setup:a-merge-carries-the-trunk-it-was-tested-against`. Separate create and update endpoints are why a step resolves the ruleset by name and then chooses between them, rather than blindly creating. Distinct bypass actor types are what let the administrator push without exempting the release App from strict freshness. The ruleset-level scope of a bypass is why the rules an actor may be excused from and the rules that hold against everyone are installed as two rulesets, and why the safety ruleset's empty bypass is written by the step rather than read from a configuration key.
...
## What an OpenSSF Scorecard run scores under this convention

Checked 2026-09-12. Bearing: the Scorecard paragraphs in `forges/github.md`.

Branch-Protection reads GitHub branch protection or repository rules, and the check documentation states that every setting it queries "[is] accessible via Repo Rules", so this convention's rulesets are readable. Its scoring is cumulative: "You receive partial credit for requirements met within your current tier. However, a tier must be fully satisfied before you can earn points from the next tier." Tier 1, three points, is prevent force push and prevent branch deletion. Tier 2, six points, is at least one required reviewer plus three requirements marked "For administrators": required pull requests, branch up to date before merging, and approval of the most recent reviewable push. Tier 3, eight points, is at least one passing status check. Tier 4, nine points, is at least two reviewers and review from code owners. Tier 5, ten points, is two further administrator requirements, dismissing stale reviews and including administrators in review. The admin-token note settles which of these a landed target has to satisfy itself: "If Scorecard is run without an administrative access token, the requirements that specify 'For administrators' can be safely ignored, and scores will be determined as if all such requirements have been met." The same note states that `EnforceAdmins` "is calculated as `false` if any Bypass Actors are defined on any rule, regardless of if they are admins".

Pinned-Dependencies "works by looking for unpinned dependencies in Dockerfiles, shell scripts, and GitHub workflows which are used during the build and release process of a project". The dependency kinds it classifies are declared in the checker's own types, and `DependencyUseTypeDownloadThenRun`, wire value `downloadThenRun`, is "a download followed by a run". Its evaluation weights GitHub-owned actions at 2 and third-party actions at 8 inside the actions ecosystem, and gives every other kind a normal weight of 10, so a `downloadThenRun` instance is scored beside the actions rather than under them.
