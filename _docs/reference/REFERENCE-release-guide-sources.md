...
Bearing: setup.md step 13.3, every field the browser flow enumerates. The frontend source is the authority the deployed form is built from; the blog post introduced the scoped-token UI and corroborates the field set.
...
Bearing: setup.md step 0's verified-email prerequisite, step 13.4's `cargo login` on stdin, and step 15's host-side revocation of the local copy.
