...
#### Scenario: A lock path names another filesystem object

- GIVEN a lock pathname that is a link or another non-regular entry
- WHEN an apply tries to take its target
- THEN it refuses before staging, the named object stays unchanged, and the lock namespace keeps owner-only permissions

Verify: `cargo nextest run -E 'test(an_interrupted_apply_leaves_each_destination_whole_and_journals_it) or test(the_record_is_written_last) or test(the_record_is_the_last_operation) or test(postconditions_run_and_a_failure_is_reported) or test(an_apply_lands_in_the_runs_journal) or test(the_apply_exit_codes_match_the_matrix) or test(a_second_apply_against_one_target_refuses_while_the_first_holds_it) or test(one_run_holds_a_target_at_a_time) or test(an_apply_refuses_when_it_cannot_take_the_target) or test(a_host_with_no_state_root_refuses) or test(a_lock_file_a_killed_run_left_behind_blocks_no_apply) or test(a_lock_file_without_a_live_holder_is_taken_over) or test(a_symlink_lock_file_is_refused_without_touching_its_target) or test(a_non_regular_lock_entry_is_refused) or test(the_lock_namespace_and_file_are_owner_only) or test(a_symlink_lock_namespace_is_refused)'`
