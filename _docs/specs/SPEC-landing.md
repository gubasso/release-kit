...
The routing block MUST state that an agent acting in the target guides and never drives: that a request to change code authorizes the file changes alone, and that the bounded actions — create, switch, or delete a branch, create or remove a worktree, commit, push, tag, and open, update, or merge a pull request — happen only where the operator's request named that action. It is the one landed line no mechanism enforces — a hook and a forge protection bound the end state and cannot tell an agent from a person — so the target carries it as a sentence rather than leaving an agent to discover it by refusal. Every line of the block MUST also read as plain prose — no semicolon joining two statements, no dash splicing two statements, and no sentence over 25 words — because the target owns none of these lines, so a prose gate the target runs over its own `AGENTS.md` finds a defect it cannot repair and the renderer is the only place that can answer it.
