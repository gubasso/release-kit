...
- A target that already releases somehow, and its verdict before anything lands: `rk assess`, with `rk guide migration` as the procedure and `rk method migration` as its why. The plan every landing write comes from: `rk reconcile plan` computes and stores it, `rk reconcile show` renders it, `rk reconcile apply` executes it or refuses naming what moved, and `rk init`, `rk upgrade`, and `rk adopt` are fronts over that path, with `rk guide reconcile` as the procedure and `rk method reconcile` as its why, bound by `_docs/specs/SPEC-reconcile.md`; `--to <selector>` names the release and `--decide <id>=<answer>` selects an answer.
