...
- A target that already releases somehow, and its verdict before anything lands: `rk assess`, with `rk guide migration` as the procedure and `rk method migration` as its why. The plan every landing write comes from, computed and printed and never persisted: `rk reconcile plan`, bound by `_docs/specs/SPEC-reconcile.md`; `--to <selector>` names the release and `--decide <id>=<answer>` selects an answer.
