...
- `snippets/` is scoped by the capability that lands each file: the release automation at one `(driver, forge)` pair under `snippets/<driver>/<forge>/`, and the forge-scoped capabilities under `snippets/_shared/<forge>/`; `src/profile/catalog.rs` is the one owner of which file belongs to which capability, and a pair may honestly land fewer files than another.
- `snippets/_shared/<forge>` holds a forge's technology-independent files, each owned by one capability the catalog selects; it is not a technology, and a destination it shares with a pair is a projection defect.
...
- Every landable file has a declared kind in `src/landing.rs` — `rendered`, `seeded`, or `state` — and a landing writes `.release-kit/manifest.json` into the target, last. `SPEC-landing.md` binds the record and every verb that reads it. The target configuration is typed by domain: `src/profile.rs` resolves the project profile, the Git workflow, and the capability requests by one precedence, `SPEC-project-profile.md` and `SPEC-git.md` bind them, and `rk profile` reports what a target resolves to.
...
- The worktree lifecycle and the checkout mode: `rk worktree`, with `rk guide worktree` as the procedure. The authority that carries an implementation onto the trunk is the other Git workflow axis, `git.integration`, `local` or `forge`: `rk integrate <branch>` performs it, `rk upgrade --integration <mode>` changes it, `rk guide integration` is the procedure and `rk method integration` its why. This repository records `local`.
...
- The committed target answers: `.release-kit/config.toml`, the target configuration, read by landing verbs and written before the manifest; `_docs/specs/SPEC-target-config.md` binds the file, its floors, and informational pending input, `_docs/specs/SPEC-project-profile.md` binds the profile and the capability selection, and `_docs/specs/SPEC-git.md` binds the Git workflow. What a target resolves to, with each value's source and the capabilities the catalog selects: `rk profile`, which writes nothing. What a landed target reports about itself: `rk status`, with `--check` as the judging mode; `rk upgrade` takes it to the installed binary's projection; `rk adopt` records a pre-record target.
...
- Work reaches the trunk as one squash commit that `rk integrate <branch>` writes from a short-lived branch, once the `manual` stage passes. The branch name is `<type>/<slug>`, whose type matches the squash title's type, or the forge-minted `<issue-id>-<slug>`. Nothing else commits on `master`, pushing the trunk is a separate operator action, and `--forge` sends one execution through a pull request instead.
