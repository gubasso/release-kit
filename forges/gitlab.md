...
Vulnerability reports use confidential issues, with confidentiality selected by the reporter and viewing governed by project access. The required `private-vulnerability-reporting` step reads issue intake configuration without changing it and names the limitation: GitLab has no project-level private reporting switch, and feature access cannot prove every external reporter can submit. The landed `SECURITY.md` describes the procedure on the project's own instance. Where the confidential issue form is unavailable the policy names a fallback contact, which `security.contact` in `.release-kit/config.toml` supplies; empty keeps this forge's own wording, an existing private conversation with a maintainer. `security.response` states the acknowledgment window the same policy promises.
...
- The policy is floored here too. `protection.gitlab.merge_method` is `ff`, `squash_option` is `always`, `push_access_level` is zero, and `squash_commit_template` must reference `%{title}`; the template names no `%{description}`, because the merge request's description does not reach the trunk on this forge. `merge_access_level` floors at thirty and defaults to forty, so a project may loosen it to a developer level deliberately and never by omission.
- GitLab has no ruleset, so `protection.trunk_ruleset`, `protection.tag_ruleset`, `protection.lines_ruleset`, and `protection.title_check` name nothing here and no step reads them. A protection is addressed by the branch or tag pattern it covers, which comes from `project.trunk` and `setup.line_prefix`. The parity rule binds step names rather than variables, so a variable one forge alone consumes breaks nothing.
