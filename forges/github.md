...
Branch-Protection scores in cumulative tiers, so an unsatisfied tier forfeits every tier above it, and a run from a workflow token treats each requirement the check marks "for administrators" as met. The landed safety ruleset refuses deletion and force-push, which is tier one and three points, and the trunk ruleset requires two status checks, which is tier three. Between them sits the one tier-two requirement this convention leaves to the target: at least one required reviewer. `protection.required_approving_review_count` floors at zero, so a target that keeps the default reads three out of ten rather than eight, and it is not misconfigured. One required reviewer reaches eight. Ten needs two reviewers and a code owner review, which a solo-maintained project cannot honestly require. Forge integration's empty `bypass_actors` list satisfies the tier-five no-excluded-administrator requirement. Local integration deliberately trades that tier away: its sole repository-administrator bypass admits the direct trunk push while leaving integrations governed.
...
`rk init --code-scanning codeql` lands `.github/workflows/code-scanning-codeql.yml`, and `--code-scanning semgrep` lands `.github/workflows/code-scanning-semgrep.yml` beside it as the alternative. One provider lands one destination, so switching retires the other and leaves its file as the target's own. Both are `rendered`. Neither triggers on a pull request, and that is the rule this forge states above rather than a limitation of the workflow: `needs` resolves inside one workflow file, so a second request-reporting workflow holds no merge whatever it proves. Each uploads its SARIF to this repository's code scanning, so the job takes `security-events: write` and the workflow level takes nothing. It also takes `actions: read`, which only a private repository needs and without which the upload fails there: the action reads the workflow run it reports against. The landing does not know whether the target is private, and read access to a repository's own runs costs a public target nothing. A private target fetching private CodeQL packs adds `packages: read` beside it; the default query packs need none. Neither adds a setup step. A scanner reads one language, so both workflows ship in the binding's own pair rather than in this forge's shared zone, and a binding that ships none refuses the flag by name. `codeql` carries a licence condition the binding owns; `semgrep` carries none. [The Rust binding](../bindings/rust.md) states which field the licence is read from and which three providers were refused.

`rk init --scorecard` lands that workflow, and the landing records the answer as `capabilities.scorecard` in `.release-kit/config.toml`. What the target gets is `.github/workflows/scorecard.yml`, a `rendered` file release-kit owns: it runs on a push to the recorded trunk and once a week, computes the score, and publishes it to the public Scorecard API, which is what makes the badge and the site entry resolve. It uploads no SARIF, so it needs neither code scanning nor `security-events: write`, and it adds no setup step, because a run needs no forge setting on a public repository. The capability is off by default: a run posts to a public API and reads repository metadata, which is a choice rather than a guard. `rk upgrade --scorecard off` drops the destination from the receipt and leaves the file as the target's own. `ossf/scorecard-action` publishes no moving major tag, so `versions.toml` pins the exact release tag and only a retag moves the discovery ref.
...
- The gate is the release request's own merge, enforced by the trunk's ruleset: squash as the only merge method and two required status checks matched by name — the one job the project's own workflow reports as its gate, and the landed `pr-title` check. Under forge integration no direct push bypass exists. Under local integration only the repository-administrator role bypasses those rules for the deliberate push; the release App remains governed, and the deletion and force-push rules sit in a second ruleset nobody bypasses. The request must also carry the trunk's tip: `strict_required_status_checks_policy=true` enforces freshness at merge time in both modes, and `rk setup check` faults a false or absent policy. `needs` names job ids inside one workflow file, so that gate reaches no job in another file: any second workflow reporting on a request gates nothing, whatever it proves, which is why the landed workflows report on a request from the title check alone and a generated artifact workflow is configured to report on none.
...
- This convention refuses a merge queue, and the aggregate gate is what it uses instead. A queued repository holds every request until a check that triggers on `merge_group` reports, and the landing ships no workflow that does, so the queue waits on a check that never comes and drops the request when its CI timeout expires. `rk setup check` faults a queue on the trunk by name and states that remedy: `rk setup step protect-trunk --apply` rewrites the ruleset without it. A project that wants a queue wants a different convention, not a different flag.
- Protections are rulesets: two over the trunk, one over the `v*` tags, and — where a project keeps older lines — one over the `release/*` branches. A ruleset has separate create and update endpoints, so a rerun resolves the ruleset id and updates in place.
- The trunk takes two because a bypass actor is a ruleset field and excuses every rule in that ruleset. The safety ruleset carries deletion and force-push protection and names nobody; the trunk ruleset carries the request rule and the required checks and names whoever the integration mode excuses. `protect-trunk` writes the safety ruleset first, so no rerun leaves a window with neither, and it writes that empty bypass itself rather than reading a key, so no configuration answer can widen who may rewrite the trunk.
- A ruleset's name is the project's own. `protection.trunk_ruleset`, `protection.safety_ruleset`, `protection.tag_ruleset`, and `protection.lines_ruleset` name the four, and the setup and its check read the same value, so they cannot disagree about what a ruleset is called. A target that names no trunk ruleset gets `<trunk>-protection`, which is what the step built before the key existed, and one that names no safety ruleset gets `<trunk>-safety`. Renaming a ruleset after it exists leaves the old one standing: the forge resolves by name, so the setup creates a second ruleset rather than renaming the first, and the operator deletes the old one.
- The protection's policy is floored rather than free. `protection.tag_pattern`, `bypass_actors`, `allowed_merge_methods`, `strict_required_status_checks`, `owned_trunk_rules`, and the two squash sources each carry a minimum the method states, and a weaker value is refused naming the key, the floor, and `rk method invariants`. The bypass floor is mode-dependent: empty under forge integration and exactly `repository-admin` under local integration. The review block floors permissively: a project may require approvals and may dismiss stale reviews, and the floor exists to say which direction is safe. `rk` judges every value before a step runs, so a step substitutes and never decides.
- `protection.title_check` names the context the trunk protection requires for the landed title job. The job's own name inside `pr-title.yml` stays a constant of the rendered file, so a target that renames the key without renaming the job requires a check nothing reports, which hangs the merge. `rk setup check` reports that as a fault rather than passing.
