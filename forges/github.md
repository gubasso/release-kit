...
`rk init --code-scanning codeql` lands `.github/workflows/code-scanning-codeql.yml`, and `--code-scanning semgrep` lands `.github/workflows/code-scanning-semgrep.yml` beside it as the alternative. One provider lands one destination, so switching retires the other and leaves its file as the target's own. Both are `rendered`. Neither triggers on a pull request, and that is the rule this forge states above rather than a limitation of the workflow: `needs` resolves inside one workflow file, so a second request-reporting workflow holds no merge whatever it proves. Each uploads its SARIF to this repository's code scanning, so the job takes `security-events: write` and the workflow level takes nothing. Neither adds a setup step. `codeql` carries a licence condition the binding owns; `semgrep` carries none. [The Rust binding](../bindings/rust.md) states which field the licence is read from and which three providers were refused.
