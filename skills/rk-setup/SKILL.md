...
6. Decide the workflow mode in the same planning turn, with `AskUserQuestion` beside the binding and forge choices: `--workflow worktree` (the default, recommended — every code-changing branch in a linked worktree, the main checkout commits nothing) or `--workflow branches` (branches worked in the main checkout, worktrees optional beside them). The answer flows into the planned `rk init` flags; `rk method worktrees` owns the trade.
...
10. Walk the repository-side setup by the guide's numbers: `rk guide setup` steps 1 to 3 for the forge-side commands and steps 5 to 8 for the registry-side actions, with `rk method setup` for each step's reasoning. The bot-identity walkthrough is `rk forge <name>`.
