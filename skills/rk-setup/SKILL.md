...
6. Decide the workflow mode, the release style, and the Nix opt-in in the same planning turn, with `AskUserQuestion` beside the binding and forge choices; the answers flow into the planned `rk init` flags.
