...
`rk guide` substitutes what detection knows — the project path as `<repo>`, the technology as `<tech>` — and nothing else. Blocks labeled `On github:`, `On gitlab:`, `On rust:`, `On python:`, `On bash:`, `On worktree:`, or `On branches:` are variants: when the axis is resolved, the matching block is kept and its siblings are dropped; unresolved, every variant prints with its label. The workflow axis resolves from the landing record's recorded mode, with `--workflow` as the override; the forge and technology axes resolve by detection. Placeholders such as `<release pr>` exist only once a bot has opened them and are never substituted: a stale number merges someone else's work, where a visible placeholder fails loudly.
