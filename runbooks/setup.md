...
`rk setup check --required-check <name>` names the jobs the gate leaves out, and a gate without `if: always()`, as a limitation on `protect-trunk`. What to write instead is the binding's, and `rk status --check` names it by remediation for a generated workflow.
