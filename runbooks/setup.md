...
Creating the bot App itself needs a browser, because the manifest flow redirects through one; it happens once in an account's lifetime, never per project. `rk forge github` carries the walkthrough, which credentials to collect, and the warning about the private key that downloads exactly once. With the App created, export `RK_BOT_APP_ID` and `RK_BOT_PRIVATE_KEY_FILE` (the path to the `.pem`, kept at mode `600` outside the repository) and the steps are commands: `rk setup step install-bot --target . --apply` asserts the installation covers this project, and `rk setup step bot-secrets --target . --apply` stores the credentials. `install-bot` observes as the App itself with those two exports; only granting a repository the installation does not yet cover needs `GH_TOKEN` holding a classic personal access token with `repo` scope — the one write GitHub offers a command — or one click at `github.com/settings/installations` grants it by hand; `rk forge github` states the caveat.
