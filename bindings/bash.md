...
- On GitHub, the landed `release.yml` attests the tarball with `actions/attest` in the same job that builds it, which is why `tag-and-attach` carries `id-token: write`, `attestations: write`, and `artifact-metadata: write` alongside `contents: read`. The attestation is minted before `gh release create` runs: the order is the rule, not an implementation detail, because the release page is public the moment it exists, and a page created first would point at an unattested tarball until the attest step ran — permanently, if the run failed between the two. A consumer verifies with `gh attestation verify <tarball> --repo <owner>/<repo>`. An `install.sh` served from the release page is itself an artifact and can be attested and verified the same way before it is run.
