...
1. Record the style. A project that keeps older lines lands `--release-style lines`: its release request is never armed, because a line's release is a promotion of something a human validated, and `rk status` is where a clone, a CI job, and an agent read that answer.
2. Wire the repository for lines, once: `release/*` takes no force-push and no deletion while a line is alive, and the landed release workflow already watches `release/**`, so a line gets its own release request. The project's own CI is the second half of that wiring and release-kit writes none of it: a pull-request filter selects by base branch, so a request opened against a line starts no run of a workflow naming the trunk alone, and the check the trunk protection requires never reports on the line.
