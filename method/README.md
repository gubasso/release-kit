...
Start with [the model](./00-model.md), hold [the invariants](./01-invariants.md), then follow the chapter for the work at hand: [setup](./02-setup.md) once per repository, [operate](./03-operate.md) for every release, [recovery](./04-recovery.md) when one goes wrong. [The diff surface](./05-diff-surface.md) is where the bindings plug in, and the two walkthroughs — [release from trunk](./06-release-from-trunk.md), [branch for release](./07-branch-for-release.md) — show each release style end to end. [Worktrees](./08-worktrees.md) owns the working-copy forms and the workflow mode that picks between them, [release lines](./09-release-lines.md) owns a line's whole life, and [migration](./10-migration.md) owns the target that already releases somehow — the verdict, the findings inventory, and the loop that carries it across. [Dependencies](./11-dependencies.md) owns how a project takes another project as a dev or prod dependency, from the source's own distribution and the target's own tool manager. [Reconcile](./12-reconcile.md) owns how every release-kit write reaches a repository: one plan, one decision, one apply, shared by setup, migration, and upgrade.
...
| [12 — Reconcile](./12-reconcile.md)                   | The plan, the decisions, and the apply behind every landing write |
