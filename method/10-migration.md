...
A recorded landing is a fourth state and not a classification: it routes to `rk status`, and from there to [reconcile](./12-reconcile.md), whose plan classifies it as an upgrade or as the drift the report names. No record does not mean no release: a brownfield target with no record is the common case, and treating it as green is how a repository ends up with two release paths at once.
