...
How a repository moves from wherever it releases today to this convention, without losing a release path and without a step nobody can retrace. [Setup](./02-setup.md) bootstraps a bare repository once, in order; this chapter owns the target that already ships somehow — a release tool of its own, a gated two-branch flow, files landed before the record existed, or a landing that drifted — and the loop that carries it across. The command form of this chapter is the migration runbook, `rk guide migration`; `rk assess` reads the verdict the first step depends on.
...
- greenfield: no release mechanism and no release history — no tool configuration, no landed destination on disk, no tag, no second long-lived branch. Land the workflow through [setup](./02-setup.md); there is nothing to migrate.
- brownfield: a release mechanism is in place — another tool's configuration, a landed destination already present, a landed block. The mechanism is migrated, never overwritten and never left as a second release path beside the new one.
...
A recorded landing is a fourth state and not a classification: it routes to `rk status`, and from there to [landing](./13-landing.md), whose stage shows what an upgrade would replace and what the report names as drift. No record does not mean no release: a brownfield target with no record is the common case, and treating it as green is how a repository ends up with two release paths at once.
...
1. Classify the target and read what it runs. `rk assess` gives the verdict and the evidence; `rk status` the landing and its drift; `rk setup check` what the forge enforces; `rk versions --check` the pins; `rk self-depend status` how the project obtains `rk` and what a predecessor mechanism left. Every later step is a gap one of these reported, and every step ends by rerunning the observation that found it.
2. Record the landing. A target with no record is verified against one rendered candidate and recorded by `rk adopt`, after its marked blocks are brought to the candidate's bytes; a recorded target older than the binary, or missing a parameter the record now carries, is taken forward by `rk upgrade`. Both render afresh from the installed binary, and [landing](./13-landing.md) owns the stage the agent studies first, the ownership words the verb prints, and the refusal it gives an unattributed file. The hooks are settled before the block lands, the sentinels are answered from the project, and the project's own checks prove the landed files on whatever branch is the default today. A landed configuration another tool consumes is regenerated before that proof: a brownfield target arrives with the tool's previous output already committed, and the forge executes that output rather than the configuration behind it, so a landing nobody regenerated took no effect where it counts.
...
7. Close on evidence. `rk status --check`, `rk setup check`, `rk versions --check`, and `rk self-depend status` read green together, the hooks are installed in the clone, and the first release cut through [operate](./03-operate.md) is the migration's proof — not the last checked entry.
...
A checkout-mode or release-style change is a named migration: edit `git.checkout_mode` or `profile.release.style` in `.release-kit/config.toml`, preview with `rk upgrade`, and apply with `rk upgrade --apply` through the trunk’s one path. A flag overrides the configured key and an apply writes it back while preserving comments. [Worktrees](./08-worktrees.md) owns the transition for branches open across a mode change, and [release lines](./09-release-lines.md) owns what the style changes; this chapter adds only that the change is an inventory entry with its own verification, not a line in another entry.

An existing target with no config keeps its recorded parameters. Its first upgrade adds the config from those parameters and compiled defaults, with every landed byte unchanged where the projection itself is unchanged. A pre-style record needs the config or a flag to answer style; an omitted key still leaves that decision open. Adoption verifies the candidate first, then writes the config and the record inside `.release-kit/`, leaving landed destinations untouched.
