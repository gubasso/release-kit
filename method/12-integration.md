...
Under local integration GitHub keeps every trunk rule, including the request rule and strict required check, and holds them in two rulesets. The safety ruleset carries deletion and force-push protection and names no bypass actor. The trunk ruleset carries the request rule and the required check, and grants exactly the repository-administrator role an always bypass for the deliberate direct push. The split is not decoration: a bypass actor is recorded on a ruleset and excuses every rule in it, so one ruleset carrying all four would hand the operator the force-push and the deletion along with the push. The release App is a distinct integration actor, holds no repository role, and reaches neither bypass, so its release request still cannot merge after the trunk advances. GitLab carries the local push authority through `protection.gitlab.push_access_level`. `rk setup` installs the shape the recorded mode names and its check expects that same shape, so a target that records `local` is neither locked out of its own integration nor stripped of atomic release freshness. Every tag protection is unchanged in both modes.

The local-mode release workflow retries the merge but is not the atomic protection. It wakes when the named workflow completes and after its push-side request job succeeds, discovers candidates across every result page, proves the request's identity against the forge, and attempts the merge only once the same recorded check has concluded successfully for the exact head. Any other answer is a successful no-op that leaves the request open, and so is a merge the trunk's own rules refuse: the job names what the forge answered and ends green, because a stale request withheld is the protection working. The ruleset performs the final base-freshness judgment at the merge boundary.
