...
Before the first step the project decides how it obtains `rk`: one host install for the whole machine, or its own devshell pin — release-kit as a flake input at a release tag, the tag in `flake.nix` the version and the lock its content, moved forward together by `rk self-depend sync` from `.envrc` once a day. One project runs one bump mechanism, because two mechanisms write the same two files on the same trigger with two locks that do not know each other, and the second to run either fights the first or silently undoes it; so the wiring replaces what the project carried, `rk self-depend clean` removes what a line scan can judge and names the rest, and a target reads as ready only with nothing of the predecessor left.

The decision is asked rather than assumed, and it is two questions rather than one: which manager the project runs, and what moves the pin that manager records. A pin with nothing moving it, and a project with no pin at all, are both the state this chapter exists to prevent, and a setup that never raises either question reaches them through the supported path. So a setup asks both once, and records what comes back: a refusal is a decision the project made, never a step nobody reached.
...
Two protections owned by every repository, a merge-permission switch and vulnerability intake channel beside them, and a third protection where older lines exist, all held by configuration that a script can verify:

- `master` takes no direct push and no force-push, requires a pull request carrying the named passing check and the landed title check — one named check, so a multi-job workflow names a gate that needs every job whose failure must hold the merge, and a proof in a second workflow file is one that gate cannot need, so a technology whose tooling generates its own workflow configures it to report on no request and the project's gate runs the proof — and offers squash as the only merge method, with the squash message taken from the request's title — asserted, because the forge's one-commit fallback is the branch commit's own subject.
...
- The forge's own version is a precondition of this protection. Where the convention's landed pipeline needs a feature the forge added in a known version, an instance below that version cannot honor the merge check this section installs, so the setup reads the version first and refuses the protection rather than installing one the pipeline cannot feed. A version that cannot be read blocks the same as one that is too old: a floor nobody could read proves neither way. That forge's document names the version and the feature.
- The trunk's requests may merge themselves once every required check passes. The switch is what the trunk style's standing release instruction needs, and it is asserted rather than assumed because a repository that forbids it fails silently: the request simply never offers the option. The forge that carries no such switch says so, and its pipeline requirement is what stands in.
...
- A vulnerability report needs both the forge's private channel and the reporting policy landed in step 4: a setting gives a reader no instructions, and a file alone enables no channel. Private GitHub repositories are inapplicable; a forge proving less names its limitation. The forge documents name the guarantees, and `rk guide setup` step 3g orders the check.

## 4. Land the workflow files

`rk init` lands the binding's files: the bot configuration, the publish workflow, the artifact-builder configuration, the reporting policy, the title check, and the commit-contract hook block spliced into `.pre-commit-config.yaml`. [Landing](./13-landing.md) owns how the write reaches the repository: the stage, the investigation, the fresh render, the verification, and the cleanup. This step owns what the landing carries and why. It lands no CI workflow, and the title check is the one landed file that reports on a request, because it is the second context the protection requires by name; every other proof is a job of the project's own gated pipeline, which the binding serves and the project writes. The landing records no scope vocabulary: the title check holds a scope to one shape — lowercase letters, digits, and `_ . / -` — the commit hook requires that a scope is there, and the routing block tells the author to name the area changed and to prefer a word the repository's own history already carries. A project's areas grow without a re-landing, and a scope nobody has used before is a decision at the desk rather than a round trip through release-kit. It records the workflow mode — `worktree`, the default, guards the main checkout the way this section's hooks guard the commit contract, and `branches` leaves both working-copy forms open; [worktrees](./08-worktrees.md) owns the choice, and `rk upgrade --workflow <mode>` changes it later through the same landing machinery. It records the release style the same way — `trunk`, the default, arms the bot's request so a green trunk ships itself, and `lines` keeps the merge a per-release act because a line's candidate is validated by a human; [release lines](./09-release-lines.md) owns the choice, and `rk upgrade --style <style>` changes it later. Fill the sentinel placeholders it reports, install the hook stages the block's first line names, and hold the invariant that the publish workflow filename is the one the registry will be told. The hooks and step 3's forge protections are the same rules at two distances: the forge enforces, the hooks give the refusal at the desk, and `--no-verify` bypassing a hook changes what an author sees, never what the forge accepts.
